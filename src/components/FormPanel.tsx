...
export function FormPanel({
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-card sm:p-6">
  ...
  return (
    <label className="grid min-w-0 max-w-full gap-1.5 text-xs font-bold leading-4 text-slate-700 sm:gap-2 sm:text-sm">
      ...
      {type === "file" ? (
        ...
        <ValidatedInput name={name} className={inputClassName} type={type} placeholder={placeholder} validation={validation} defaultValue={defaultValue} />
      )}
    </label>
  );
}
