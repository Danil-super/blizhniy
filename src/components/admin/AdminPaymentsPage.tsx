        ...
        <tbody className="divide-y divide-slate-100">
          {payments.map((payment) => (
            <tr key={payment.id} className="align-top transition hover:bg-blue-50/40">
              ...
              <td className="px-4 py-4 text-xs font-bold text-slate-600">{payment.provider === "yookassa" ? "ЮKassa" : "Тест"}</td>
...
export async function AdminPaymentsPage() {
  ...
  const totalSucceeded = payments.filter((payment) => payment.status === "succeeded").reduce((sum, payment) => sum + payment.amount, 0);
  const pendingCount = payments.filter((payment) => payment.status === "created" || payment.status === "pending").length;
