...
export default async function CreateVacancyPage({ searchParams }: PageProps) {
  ...
  async function publishVacancyWithoutPaymentAction(formData: FormData) {
    ...
    if (!captchaVerified) {
      redirect(`/rabota/vakansii/sozdat?admin=1&error=${encodeURIComponent(TURNSTILE_ERROR_MESSAGE)}`);
    }
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {adminMode ? (
                    ...
                    <AdminDemoPublishButton
                      ...
                      buttonClassName="inline-flex h-12 w-fit items-center justify-center rounded-xl bg-[#d92d20] px-7 font-bold text-white transition hover:bg-[#b42318] disabled:cursor-wait disabled:bg-slate-300"
                    />
                  )}
                </div>
              </VacancyEmployerFields>
