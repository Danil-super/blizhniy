...
function validationError(message: string) {
  redirect(`/rabota/specialisty/anketa?error=${encodeURIComponent(message)}`);
}
...
export default async function SpecialistProfileFormPage({ searchParams }: PageProps) {
  ...
  async function publishSpecialistWithoutPaymentAction(formData: FormData) {
    ...
    if (!specialistId) {
      ...
      if (!captchaVerified) {
        redirect(`/rabota/specialisty/anketa?error=${encodeURIComponent(TURNSTILE_ERROR_MESSAGE)}`);
      }
    }
